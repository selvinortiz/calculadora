...
export async function POST(request: NextRequest, { params }: { params: Promise<{ transactionId: string }> }) {
  ...
  const session = await getCurrentPortalSession(); const supabase = await createSupabaseServerClient();
  ...
  const { data, error } = await supabase.rpc("void_transaction", { target_transaction_id: transactionId, reason });
