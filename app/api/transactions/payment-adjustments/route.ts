...
export async function POST(request: NextRequest) {
  ...
  if (!Number.isInteger(paymentNumber) || paymentNumber < 1 || !Number.isInteger(expectedLoanVersion) || !Number.isFinite(receivedPayment) || !isIsoDate(body.paymentDate) || !isIsoDate(body.nextPaymentDate)) return mutationError("validation", "Revisa los datos del ajuste.");
  ...
  const command: PostPaymentAdjustmentCommand = {
    ...
    paymentReference: text(body.paymentReference, 120), adjustedBy: text(body.adjustedBy, 80), notes: text(body.notes, 1000),
    snapshot: { version: DOCUMENT_SNAPSHOT_VERSION, calculationVersion: CALCULATION_VERSION, documentKind: "payment_adjustment_record", issuedAt: `${body.paymentDate}T00:00:00.000Z`, organizationName: session.company, customerName: customer?.name || "Cliente", accountReference: loan.account_reference, payload: { adjustment, paymentDate: body.paymentDate, nextPaymentDate: body.nextPaymentDate, paymentReference: text(body.paymentReference, 120), adjustedBy: text(body.adjustedBy, 80), notes: text(body.notes, 1000), notice: "Este ajuste se aplica únicamente a la cuota siguiente." } },
    ...(replacesTransactionId ? { replacesTransactionId } : {}),
  };
  const { data, error } = await supabase.rpc("post_payment_adjustment", { command });
