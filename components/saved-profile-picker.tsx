...
export function SavedCustomerPicker({ onSelect, value }: { onSelect: (customer: DirectoryCustomer | null) => void; scope?: string; value: string }) {
  ...
  return <div className={`${styles.picker} ${styles.compact}`}><div className={styles.labelRow}><label htmlFor="saved-customer">Cliente</label><Link href="/clientes">Ver todos</Link></div><select id="saved-customer" value={value} disabled={!isReady || customers.length === 0} onChange={(event) => onSelect(customers.find((item) => item.id === event.target.value) ?? null)}><option value="">{customers.length === 0 ? "No hay clientes" : "Seleccionar"}</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></div>;
}
